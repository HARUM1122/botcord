    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          () {
            List<(String, String, AuditLogEvent?, Future<(User, String, List<String>)> Function(AuditLogEntry, Guild?)?)> filteredActions = actions.values.where((element) => element.$1.toLowerCase().contains(_actionName.toLowerCase())).toList();
...
class ActionWidget extends ConsumerWidget {
  final Icon leading;
  final (String, String, AuditLogEvent?, Future<(User, String, List<String>)> Function(AuditLogEntry, Guild?)?) action;
  final bool selected;
  const ActionWidget({
